...
func _ready():
	...
	PixelPen.project_file_changed.connect(func():
			if PixelPen.current_project != null:
				animation_fps.value = PixelPen.current_project.animation_fps
			create_frame_list()
			)
	PixelPen.layer_items_changed.connect(create_frame_list)
	playback_timer.timeout.connect(_on_timer_timeout)
	animation_fps.value_changed.connect(func(value):
			if PixelPen.current_project != null:
				PixelPen.current_project.animation_fps = value
			)
	...
	if PixelPen.current_project != null and PixelPen.current_project.animation_is_play:
		...
		for i in range(children.size()):
			...
			if path != null:
				...
				if i == PixelPen.current_project.animation_frame_index:
					label_style.bg_color = PixelPen.userconfig.accent_color
					label.label_settings.font_color = Color.BLACK
				else:
					label_style.bg_color = PixelPen.userconfig.layer_body_color
					label.label_settings.font_color = Color.WHITE


func play_pause():
	PixelPen.current_project.animation_is_play = not PixelPen.current_project.animation_is_play
	if PixelPen.current_project.animation_is_play and playback_timer.is_stopped():
		PixelPen.animation_about_to_play.emit()
		if PixelPen.current_project.animation_frame_index == -1:
			PixelPen.current_project.animation_frame_index = 0
		_refresh_frame()
		var frame_time = 1.0 / PixelPen.current_project.animation_fps
		_record_prev_time = Time.get_unix_time_from_system()
		_record_fps.clear()
		playback_timer.start(frame_time)
	elif not PixelPen.current_project.animation_is_play:
		playback_timer.stop()
		actual_fps.text = ""
...
func _on_timer_timeout():
	...
	if abs(PixelPen.current_project.animation_fps - fps) < 3:
		actual_fps.text = ""
	elif abs(PixelPen.current_project.animation_fps - fps) < 5:
	...
	if PixelPen.current_project.animation_timeline.size() == PixelPen.current_project.animation_frame_index:
		PixelPen.current_project.animation_frame_index = 0
		if not PixelPen.current_project.animation_loop:
			play_pause()
			return
	_refresh_frame()
...
func _refresh_frame():
	...
	var pool_index = PixelPen.current_project.get_pool_index(cell.frame.frame_uid)
	PixelPen.current_project.canvas_pool_frame_uid = PixelPen.current_project.pool_frames[pool_index].frame_uid
	PixelPen.layer_items_changed.emit()
...
func create_frame_list():
	...
	if PixelPen.current_project == null or PixelPen.current_project.use_sample:
		return
	var margin : int = 8
	...
	var create_cell = func(pool_index : int, frame_index : int , is_active : bool, left_margin : bool = false):
			...
			margin_container.gui_input.connect(func(event):
					if event is InputEventMouseButton:
						if event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT:
							...
							PixelPen.current_project.animation_frame_index = frame_index
							PixelPen.current_project.canvas_pool_frame_uid = PixelPen.current_project.pool_frames[pool_index].frame_uid
							PixelPen.layer_items_changed.emit()
							PixelPen.project_saved.emit(false)
			...
			style.bg_color = PixelPen.userconfig.canvas_base_mode_color
			
			var cell_frame_uid : Vector3i = PixelPen.current_project.animation_timeline[frame_index].frame.frame_uid
			var frame_active : bool = PixelPen.current_project.active_frame.frame_uid == cell_frame_uid
			if frame_active:
				style.border_color = PixelPen.userconfig.accent_color
			else:
				style.border_color = PixelPen.current_project.animation_timeline[frame_index].frame.frame_color
			...
			if not linked_l:
				preview.show_frame(PixelPen.current_project.pool_frames[pool_index])
	...
	var first := true
	for cell_i in range(PixelPen.current_project.animation_timeline.size()):
		var cell := PixelPen.current_project.animation_timeline[cell_i]
		var pool_index = PixelPen.current_project.get_pool_index(cell.frame.frame_uid)
		used_frame.push_back(pool_index)
		create_cell.call(pool_index, cell_i, PixelPen.current_project.animation_frame_index == cell_i, first)
		first = false
	...
	var create_unused_cell = func(pool_index : int, cell_index : int , left_margin : bool = false):
			...
			var style := StyleBoxFlat.new()
			style.bg_color = PixelPen.userconfig.canvas_base_mode_color
			var is_active : bool = PixelPen.current_project.active_frame == PixelPen.current_project.pool_frames[pool_index]
			var border_margin = 2 if is_active else 1
			style.border_color = PixelPen.userconfig.accent_color if is_active else PixelPen.current_project.pool_frames[pool_index].frame_color
	...
	first = true
	var cell_i : int = 0
	for pool_i in range(PixelPen.current_project.pool_frames.size()):
...
func _is_linked(index_a : int, index_b : int) -> bool:
	...
	if index_a < 0 or index_b == PixelPen.current_project.animation_timeline.size():
		return false
	
	if PixelPen.current_project.animation_timeline[index_a].frame.frame_uid == PixelPen.current_project.animation_timeline[index_b].frame.frame_uid:
		return true
