...
func _on_mouse_pressed(mouse_position : Vector2, callback : Callable):
	...
	(PixelPen.current_project as PixelPenProject).create_undo_layer("Move", index_image.layer_uid, func ():
...
func on_about_to_start_transform():
	_cache_undo_redo = PixelPen.current_project.undo_redo
	PixelPen.current_project.undo_redo = UndoRedoManager.new()


func on_end_transform():
	PixelPen.current_project.undo_redo = _cache_undo_redo
...
func _transform(type : int):
	...
	if PixelPen.current_project.multilayer_selected.is_empty():
		...
		for layer in PixelPen.current_project.active_frame.layers:
			if PixelPen.current_project.multilayer_selected.has(layer.layer_uid) or layer.layer_uid == index_image.layer_uid:
...
func _on_move_cancel():
	...
	if not PixelPen.current_project.multilayer_selected.is_empty() and mode == Mode.CUT:
		for multilayer_selected_layer_uid in PixelPen.current_project.multilayer_selected:
				if multilayer_selected_layer_uid != index_image.layer_uid:
					var multilayer_selected : IndexedColorImage = PixelPen.current_project.find_index_image(multilayer_selected_layer_uid)
					if multilayer_selected != null:
						multilayer_selected.colormap = multilayer_selected._cache_colormap.duplicate() 
						PixelPen.layer_image_changed.emit(multilayer_selected_layer_uid)
	...
	if index_image != PixelPen.current_project.active_layer:
		index_image = PixelPen.current_project.active_layer
	reset()
	_draw_hint(node.get_local_mouse_position())
	mode = Mode.UNKNOWN
...
func _on_move_commit():
	...
	if index_image != PixelPen.current_project.active_layer and PixelPen.current_project.multilayer_selected.is_empty():
		# UNDO
		var active_layer : IndexedColorImage = PixelPen.current_project.active_layer
		...
		_cache_undo_redo.add_undo_method(func ():
				PixelPen.layer_image_changed.emit(layer_uid)
				PixelPen.layer_image_changed.emit(layer_active_uid)
				PixelPen.project_saved.emit(false)
				)
		...
		var offset = node.overlay_hint.position
		PixelPen.current_project.active_layer.blit_color_map(move_cache_image_map, mask_selection, Vector2i(floor(offset.x), floor(offset.y)))
		...
		if not PixelPen.current_project.multilayer_selected.is_empty():
			for multilayer_selected_layer_uid in PixelPen.current_project.multilayer_selected:
				if multilayer_selected_layer_uid != index_image.layer_uid:
					var multilayer_selected : IndexedColorImage = PixelPen.current_project.find_index_image(multilayer_selected_layer_uid)
					if multilayer_selected != null:
						...
						PixelPen.layer_image_changed.emit(multilayer_selected.layer_uid)
	...
	if index_image != PixelPen.current_project.active_layer:
		...
		var layer_active_uid : Vector3i = PixelPen.current_project.active_layer.layer_uid
		PixelPen.layer_image_changed.emit(layer_active_uid)
